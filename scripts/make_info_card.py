...
portrait: colored key/value rows for experience, tech stack, and
highlights -- NOT GitHub stats (the contribution heatmap covers those).
...
ROWS = [
    ("host",),
    ("kv", "Now", "Eng. Student @ ENIG, Networks & Comms"),
    ("kv", "Prev", "Network & Systems Intern @ Tunisie Telecom"),
    ("kv", "Also", "Web Dev Intern @ Adhoc"),
    ("kv", "Edu", "B.Eng Networks & Telecom, ENIG '23-Present"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Security", "Wireshark, Nmap, Metasploit, Burp Suite"),
    ("kv", "Backend", "Node.js, Express, Laravel, Flask"),
    ("kv", "Frontend", "React, Angular, Vue.js"),
    ("kv", "AI / ML", "scikit-learn, Pandas, Random Forest"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "Built a real-time IDS with 80+ network metrics"),
    ("bul", "60% jitter/packet-loss reduction via QoS tuning"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">amin@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">amin</tspan><tspan fill="{MUTED}">@</tspan>'
